// Located at: app/api/master/upload-check/route.ts
//
// Scans every carrier=*/ folder for files matching the requested canonical label
// + month + year and returns ALL matches as an array. The frontend shows this
// list to the user, who picks a specific one to override (or cancels).
// The rename target is always plain: "{Canonical Label} {Month} {Year}.{ext}".
// Additional descriptor text like "Dental" or "Medical" in existing filenames
// is treated as noise for matching, not preserved during rename.
...
// Only the target folder matters for each canonical label. Rename uses the label
// verbatim as the base name.
...
// Strict rem/cred matching for carriers with variants. Files without an explicit
// "remittance" or "credits" keyword fall through to unknown - they don't get
...
// Scan ALL carrier folders for every file that matches the canonical label + month + year.
async function findAllMatches(
  canonicalLabel: string,
  month: number,
  year: number,
): Promise<{ key: string; size: number; lastModified: string | null; filename: string }[]> {
  const matches: { key: string; size: number; lastModified: string | null; filename: string }[] = [];
  ...
  // Sort by lastModified descending (most recent first) so the user sees the freshest one on top
  matches.sort((a, b) => {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const preferredExt = typeof ext === 'string' && ['.xlsx', '.xls', '.csv'].includes(ext.toLowerCase())
      ? ext.toLowerCase()
      : '.xlsx';

    const folder = UPLOAD_FOLDERS[canonicalLabel];
    const plannedKey = `${folder}${canonicalLabel} ${MONTH_NAMES[month]} ${year}${preferredExt}`;

    const existingFiles = await findAllMatches(canonicalLabel, month, year);
    ...
    return NextResponse.json({ exists: false, plannedKey });
  } catch (err: any) {
