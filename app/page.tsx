...
export default function Dashboard() {
  ...
  // Which billing action is in flight: the S3 key being approved, '__reset__'
  // for Reset to Default, or null when idle. A plain boolean made every row's
  // Approve button say "Approving..." at once.
  const [approvalPending, setApprovalPending] = useState<string | null>(null);
  const approvalSubmitting = approvalPending !== null;
  ...
  // ==================== APPROVAL FLOW ====================
  // Clear the approved override so the month falls back to the report our
  // reconciliation Lambda generated. Admin only, same as approving.
  async function resetBillingToDefault() {
    if (!billingSelectedMonth || approvalSubmitting) return;
    setApprovalPending('__reset__');
    setApprovalError(null);
    try {
      const res = await fetch(
        `/api/billing/approve?month=${encodeURIComponent(billingSelectedMonth)}`,
        { method: 'DELETE' }
      );
      const body = await res.json().catch(() => ({}));
      if (!res.ok) {
        setApprovalError(body.error || `Reset failed (${res.status})`);
        return;
      }
  ...
  return (
    <main className="dashboard-root">
      ...
      <style jsx>{`
        ...
        .brand-mark { width: 42px; height: 42px; border-radius: 10px; background: #ffffff; display: flex; align-items: center; justify-content: center; font-family: 'Fraunces', serif; font-weight: 700; font-size: 18px; color: #0a1628; padding: 1px; box-shadow: 0 2px 12px rgba(0, 0, 0, 0.25); }
        ...
        .brand-logo { width: 100%; height: 100%; object-fit: cover; border-radius: 9px; position: relative; z-index: 1; display: block; }
                  ...
                  <div className="billing-actions">
                    ...
                    {authUser?.isAdmin && billingReport?.approvedKey && (
                      <button
                        ...
                        title="Discard the approved file and show the report generated by the reconciliation algorithm"
                      >
                        {approvalPending === '__reset__' ? 'Resetting...' : 'Reset to Default'}
                      </button>
                    )}
                              ...
                              {u.filename && u.s3Key && (
                                <div className="update-file-row">
                                  ...
                                  {billingReport?.approvedKey === u.s3Key ? (
                                    ...
                                    >
                                      {approvalPending === u.s3Key ? 'Approving...' : 'Approve'}
                                    </button>
                                  ) : null}
                                </div>
                              )}
